...
2. **Vapi** → sign up at `dashboard.vapi.ai` ($10 free credit).
   ...
   - **Voice (Indian accent — a P0 requirement):** Vapi does **not** support
     Sarvam as a voice provider. The Indian-capable Vapi voices are **`azure`**
     (`hi-IN-SwaraNeural` for Hindi/Hinglish, `en-IN-NeerjaNeural` for Indian
     English — both bundled, no extra key), **`smallest-ai`**, or an Indian
     **`11labs`** voice. This build ships with `VOICE_PROVIDER_NAME=azure`,
     `VOICE_ID=hi-IN-SwaraNeural`. (Sarvam's excellent Hinglish is available on
     the **Bolna** path instead — see below.)
...
> **Twilio free-trial note:** a trial Twilio account prepends a spoken
> *"you have a trial account, press any key"* message and only dials *verified*
> numbers. Both are trial artifacts, not app behaviour. For a clean,
> preamble-free client demo, add a small Twilio balance, or use an Indian
> telephony provider (Exotel/Plivo) via the Bolna path.
...
| `VOICE_PROVIDER_NAME` | no | Vapi TTS provider: `azure` (default here), `11labs`, or `smallest-ai` |
| `VOICE_ID` | no | Voice id, e.g. `hi-IN-SwaraNeural` (Azure Hindi/Hinglish) |
